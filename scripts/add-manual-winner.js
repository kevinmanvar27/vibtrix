// This script manually adds a user to the winners list for a competition
// Usage: node scripts/add-manual-winner.js <competitionId> <username> <position>
...
async function main() {
  try {
    ...
    if (!competitionId || !username || isNaN(position) || position < 1 || position > 3) {
      console.error('Error: Competition ID, username, and position (1-3) are required');
      console.log('Usage: node scripts/add-manual-winner.js <competitionId> <username> <position>');
      process.exit(1);
    }

    console.log(`Adding user ${username} as position ${position} winner for competition: ${competitionId}`);
    ...
    if (!entry) {
      console.log(`User ${username} does not have an entry in the final round. Creating one...`);
    ...
    // If the user has an entry but no post, we can still set them as a winner
      ...
      console.log(`User ${username} has an entry but no post in the final round`);
      
      // Update the entry to mark it as qualified
      ...
      console.log(`Updated entry ${entry.id} to be qualified for the next round`);
    }
    ...
    const prizePosition = position === 1 ? "FIRST" : position === 2 ? "SECOND" : "THIRD";
    ...
    if (prize) {
      console.log(`Found prize for position ${position}: ${prize.amount} ${prize.currency}`);
